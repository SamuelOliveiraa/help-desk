...
export async function POST(req: NextRequest) {
	try {
		...
		if (!title || !description || !serviceID || !userID) {
			return NextResponse.json(
				{ message: "Alguns campos obrigatóriosnão foram informados. Por favor verifique." },
				{ status: 400 }
			)
		}
		...
		if (technicians.length === 0) {
			return NextResponse.json(
				{ message: "Nenhum técnico localizado. Por favor entre em contato com o administrador. " },
				{ status: 404 }
			)
    	}
		...
		if(!lastTicket){
			...
			const lastIndex = technicians.findIndex(technician => technician.id === lastTicket.technicianID)
		...
		if (!nextTechnician) {
			return NextResponse.json(
				{ message: "Não foi possível encontrar um técnico disponível no momento." },
				{ status: 503 }
			);
		}
