  ...
  try {
    ...
    if (authUser.role !== "admin") {
      return NextResponse.json(
        {
          message: "Acesso negado! Você não tem permissão para pesquisar serviços.",
        },
        { status: 403 },
      )
    }
    ...
    if (!service) {
      return NextResponse.json({ message: "Serviço não localizado" }, { status: 404 })
    }
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (authUser.role !== "admin") {
      return NextResponse.json(
        {
          message: "Acesso negado! Você não tem permissão para excluir serviços.",
        },
        { status: 403 },
      )
    }
