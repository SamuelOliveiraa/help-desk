...
import { createUser, getCurrentUser, getUsersByID, updateUser } from "@/lib/api/users"
...
export default function DetailsTechnicals({ id }: { id: string }) {
    ...
    try {
      ...
      if (user?.workingHours) {
        ...
        setWorkingHours((prev) =>
          prev.map((hour) => {
            const found = user.workingHours?.find((h) => h.id === hour.id && h.active)
            return found ? { ...hour, active: true } : { ...hour, active: false }
          }),
        )
      }
    } catch (err) {
  ...
  function handleAddWorkingHours(selectedHour: WorkingHours) {
    setWorkingHours((prev) =>
      prev.map((hour) => (hour.id === selectedHour.id ? { ...hour, active: !hour.active } : hour)),
    )
      ...
      <header>
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button loading={loading} type="submit" onClick={handleSubmit(handleSubmitForm)}>
              Salvar
            </Button>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-4 lg:flex-row h-full">
        <div className="max-w-96 w-full border border-gray-500 rounded-lg p-6 max-h-fit flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-sm text-gray-300">Defina as informações do perfil de técnico</p>
          </div>
