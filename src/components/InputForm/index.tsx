  ...
  return (
    <div className="flex flex-col gap-2 relative">
      <div className="flex flex-col-reverse relative">
        {inputID === "value" && <span className="text-base absolute top-[28px]">R$</span>}
        ...
        <input
          ...
          type={passwordView && isPassword ? "password" : !passwordView && isPassword ? "text" : ""}
          {...rest}
        />
        <label
          htmlFor={inputID}
      ...
      {!passwordView && isPassword && (
        <Eye className="absolute right-0 bottom-9 cursor-pointer" onClick={togglePasswordView} />
      )}

      {error && <span className="text-sm text-red-500">{error.message}</span>}
      {!error && helperText && <span className="text-sm text-gray-400 italic">{helperText}</span>}
    </div>
  )
}
