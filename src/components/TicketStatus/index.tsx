...
export default function TicketStatus({ variant }: { variant: Status }) {
    return (
        <div className={`w-fit flex items-center gap-2 rounded-full px-2 py-1 font-bold ${variant === 'open' ? "bg-red-400/20 text-red-400" : variant === 'finished' ? "bg-green-400/20 text-green-400" : "bg-yellow-500/20 text-yellow-600"}`
        } >
            {variant === 'open' ? (
