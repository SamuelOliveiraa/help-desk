...
export default function TicketCard({
  ...
  if (loading) {
    return <Skeleton className="w-96 h-60" />;
  }

  return (
    <Link
      href={`/dashboard/ticket/${ticket.publicID}`}
      className="max-w-96 w-full p-4 border max-h-fit rounded-lg flex flex-col gap-3"
    >
      ...
      <div className="flex flex-col gap-3 ">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-md">{formatToBRL(ticket.amount)}</span>
        </div>
      </div>
