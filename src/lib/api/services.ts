...
export async function getServices(): Promise<Service[] | null> {
  try {
    ...
    throw new Error(error instanceof Error ? error?.message : `Erro desconhecido: ${error}`)
  }
}
...
export async function deleteService(id: number) {
  try {
    ...
    const res = await axios.delete<{ service: Service; message: string }>(`/api/services/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
...
// POST criar Servico
export async function createService(data: { title: string; value: number; status: boolean }) {
  try {
    const token = await getToken()
    if (!token) return null
...
export async function updateService(data: Service): Promise<{ message: string } | null> {
  try {
    const token = await getToken()
